...
namespace edu.dts.iTunesULibrary
{
	...
	public class iTunesU
	{
		...
		{
			string authUrl = _uploadUrl + _siteDomain + ((classToken != "") ? "." + classToken : "");
			return SendAuthenticationToITunes(authUrl, GetAuthorization());
		}

		public string GetWebServicesUrl()
		{
			return _webServicesDocUrl + _siteDomain + "?" + GetAuthorization(); // +"&type=XMLControlFile";
		}
		...
		public string GetAuthorization()
		{
			...
			string signature = "&signature=" + GenerateSignature(token, _sharedSecret);
		...
		{
			return SendAuthenticationToITunes(_browseUrl + _siteDomain, GetAuthorization());
		}
		...
		private string GetSectionPath(string handle)
		{
			...
			string sectionPath = siteNode.SelectSingleNode("Name").InnerText.Replace("/", "//") + "/" +
